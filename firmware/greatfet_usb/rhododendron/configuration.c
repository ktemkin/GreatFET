...
static sgpio_function_t ulpi_register_functions[] = {
	...
	// to control the input and output timings with precision.
	{
		...
		// Synchronize our shifting to the external ULPI clock.
		.shift_clock_source           = ulpi_clock_source,
		.shift_clock_input            = &ulpi_clk_pin,

		// We shift _outputs_ on the falling edge, so the PHY can read the data
		// on its rising edge.
	...
	// as the data lines; we pre-program it to set STP at the right time.
	{
		...
		// Synchronize our shifting to the phy-generated ULPI clock.
		.shift_clock_source           = ulpi_clock_source,
		.shift_clock_input            = &ulpi_clk_pin,
	...
	// validate that our write happened correctly.
	{
		...
		// The ULPI bus specifies that data is captured on the rising edges;
		// so for _input_ data, we'll capture on the RE.
...
int ulpi_register_write(uint8_t address, uint8_t value)
{
	...
	if (register_nxt_buffer != expected_nxt_values) {
		pr_debug("rhododendron: error: ulpi reg write failed (invalid NXT states)! (expected %08x, got %08x)\n",
