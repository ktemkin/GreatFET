...
def main():
    ...
    def handle_capture_packet(packet_type, packet_data):
        ...
        names = {
            PACKET_TYPE_USB_DATA:     "DATA ",
            PACKET_TYPE_DELINEATION:  "DELIN",
            2:                        "COUNT"
        }

        print("{} PACKET: {}".format(names[packet_type], packet_data))
        return

        # If this is an "end event" packet, grab the point at which
        # we're supposed to emit the packet.
        if packet_type == PACKET_TYPE_DELINEATION:

            while packet_data:

                # Grab a single termination point from the array, and move to the next one.

                termination = int.from_bytes(packet_data[0:1], byteorder='little')
                del packet_data[0:2]

                # If this is a new sample, add it to our "emit after" list.
                if (not next_data_packet_emit_after) or (next_data_packet_emit_after[-1] != termination):
                    next_data_packet_emit_after.append(termination)
